...
passCounter = 0
failCounter = 0
frame0  = [0x5555 for i in range(834)]
print("read frame 0 status: {:d}".format(mlx.getFrame(slave_addr, frame0)))
print("subpage {:d}".format(mlx.getSubPageNumber(frame0)))
print("{:04X} {:04X} {:04X} {:04X} {:04X} {:04X} {:04X} {:04X} {:04X} {:04X}".format(frame0[0], frame0[1], frame0[2], frame0[3], frame0[4], frame0[5], frame0[6], frame0[7], frame0[8], frame0[9]))
for idx in range(834):
    if frame0[idx] != mlxTestData.Frame_0[idx]:
        failCounter += 1
        print("frame 0: {:d}".format(idx))
        print("frame 0: {:d} --- {:04X} --- {:04X}".format(idx, frame0[idx], mlxTestData.Frame_0[idx]))
    else:
        passCounter += 1
print("get frame 0 test --- pass: {:d} fail: {:d}".format(passCounter, failCounter))

# get Ta test
Tr = mlx.getTa(frame0, handle) - 8
print(Tr)

# get calculate T0 test
emissivity = 1.0
tempImg = [float(0.0*i) for i in range(32*24)]
print("get T0 image status: {:d}".format(mlx.calcT0(frame0, handle, emissivity, Tr, tempImg)))
print("{:f} {:f} {:f} {:f} {:f} {:f} {:f} {:f}".format(tempImg[0], tempImg[1], tempImg[2], tempImg[3], tempImg[4], tempImg[5], tempImg[6], tempImg[7]))
for idx in range(32):
    print("frame 0: {:d} --- {:.03f} --- {:.03f} --- {:.03f}".format(idx, tempImg[idx], mlxTestData.Frame_0_T0[idx], tempImg[idx] - mlxTestData.Frame_0_T0[idx]))
...
failCounter = 0
frame1  = list(frame0)
print("read frame 1 status: {:d}".format(mlx.getFrame(slave_addr, frame1)))
# this line is for verification of calculation frame1 = list(mlxTestData.Frame_1)
print("subpage {:d}".format(mlx.getSubPageNumber(frame1)))
print("subpage {:d}".format(frame1[833]))
# this line should not be here. It sets flag for subpage 1. It should be set by chip simulator automatically.
frame1[833] |= 0x0001
print("subpage {:d}".format(frame1[833]))
print("{:04X} {:04X} {:04X} {:04X} {:04X} {:04X} {:04X} {:04X} {:04X} {:04X}".format(frame1[0], frame1[1], frame1[2], frame1[3], frame1[4], frame1[5], frame1[6], frame1[7], frame1[8], frame1[9]))
for idx in range(834):
    if frame1[idx] != mlxTestData.Frame_1[idx]:
        failCounter += 1
        print("frame 1: {:d}".format(idx))
        print("frame 1: {:d} --- {:04X} --- {:04X}".format(idx, frame1[idx], mlxTestData.Frame_1[idx]))
    else:
        passCounter += 1
print("get frame 1 test --- pass: {:d} fail: {:d}".format(passCounter, failCounter))

# get Ta test
Tr = mlx.getTa(frame1, handle) - 8
print(Tr)

# get calculate T0 test
emissivity = 1.0
print("get T0 image status: {:d}".format(mlx.calcT0(frame1, handle, emissivity, Tr, tempImg)))
print("{:f} {:f} {:f} {:f}".format(tempImg[0], tempImg[1], tempImg[2], tempImg[3]))

for idx in range(32):
    print("frame 0: {:d} --- {:.03f} --- {:.03f} --- {:.03f}".format(idx, tempImg[idx], mlxTestData.Frame_1_T0[idx], tempImg[idx] - mlxTestData.Frame_1_T0[idx]))
