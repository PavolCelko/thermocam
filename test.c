...
int main(void)
{
    ...
    if(status == 0)
    {
        for(i = 0; i < sizeof(aEEData_test_result) / sizeof(uint16_t); i++)
            if(aEEData_test_result[i] == EEPROM[i])
                passCounter++;
            else
            {
                printf("idx %03d: %04X --- %04X\n", i, aEEData_test_result[i], EEPROM[i]);
                failCounter++;
            }
    }
    printf("EEPROM PASS: %d\n", passCounter);
    printf("EEPROM FAIL: %d\n", failCounter);


    passCounter = 0;
    failCounter = 0;
    status = MLX90640_GetFrameData(0x33, aFrame_0_test_result);    
    if(status == 0)
    {
        for(i = 0; i < sizeof(aFrame_0_test_result) / sizeof(uint16_t); i++)
            if(aFrame_0_test_result[i] == Frame_0[i])
                passCounter++;
            else
            {
                printf("idx %03d: %04X --- %04X\n", i, aFrame_0_test_result[i], Frame_0[i]);
                failCounter++;       
            }
    }
    printf("Frame_0 PASS: %d\n", passCounter);
    printf("Frame_0 FAIL: %d\n", failCounter);

    passCounter = 0;
    failCounter = 0;
    status = MLX90640_GetFrameData(0x33, aFrame_1_test_result);    
    if(status == 0)
    {
        for(i = 0; i < sizeof(aFrame_1_test_result) / sizeof(uint16_t); i++)
            if(aFrame_1_test_result[i] == Frame_1[i])
                passCounter++;
            else
            {
                printf("idx %03d: %04X --- %04X\n", i, aFrame_1_test_result[i], Frame_1[i]);
                failCounter++;
            }
    }
    printf("Frame_1 PASS: %d\n", passCounter);
    printf("Frame_1 FAIL: %d\n", failCounter);


    status = MLX90640_ExtractParameters(aEEData_test_result, &sensorParams);
    
    printf("kVdd: %d\n", sensorParams.kVdd);
    printf("KvPTAT: %f\n", sensorParams.KvPTAT);
    printf("KtPTAT: %f\n", sensorParams.KtPTAT);
    printf("vPTAT25: %u\n", sensorParams.vPTAT25);
    printf("alphaPTAT: %f\n", sensorParams.alphaPTAT);
    printf("gainEE: %d\n", sensorParams.gainEE);
    printf("tgc: %f\n", sensorParams.tgc);
    printf("cpKv: %f\n", sensorParams.cpKv);
    printf("cpKta: %f\n", sensorParams.cpKta);
    printf("resolutionEE: %u\n", sensorParams.resolutionEE);
    printf("calibrationModeEE: %u\n", sensorParams.calibrationModeEE);
    printf("KsTa: %f\n", sensorParams.KsTa);

    printf("T0T0T0T0T0T0T0T0T0T0T0T0T0T0T0T0T0T0T0T0T0T0T0\n\n");
    printf("\nT0 Frame 0\n");
    tr = MLX90640_GetTa(/*aFrame_0_test_result*/(uint16_t*)Frame_0, &sensorParams) - 8;
    printf("Tr = %f \n", tr);
    printf("SubPg myy = %d \n", MLX90640_GetSubPageNumber(aFrame_0_test_result));
    printf("SubPg mlx = %d \n", MLX90640_GetSubPageNumber((uint16_t*)Frame_0));

    MLX90640_CalculateTo(/*aFrame_0_test_result*/(uint16_t*)Frame_0, &sensorParams, 1.0, tr, pfFrameT0);

    for(i = 0; i < 32; i++)
        printf("%0.3f ", pfFrameT0[i]);
    
    memset(pfFrameT0, 0x00, 834 * sizeof(float));
    
    tr = MLX90640_GetTa(/*aFrame_0_test_result*/(uint16_t*)Frame_1, &sensorParams) - 8;
    printf("Tr = %f \n", tr);
    printf("SubPg myy = %d \n", MLX90640_GetSubPageNumber(aFrame_0_test_result));
    printf("SubPg mlx = %d \n", MLX90640_GetSubPageNumber((uint16_t*)Frame_0));

    MLX90640_CalculateTo(/*aFrame_0_test_result*/(uint16_t*)Frame_1, &sensorParams, 1.0, tr, pfFrameT0);
